...
namespace Stocks.Core.Services.FinancialStatements
{
    public class IncomeStatementService : IIncomeStatementService
    {
        ...
        public IncomeStatementService(IHttpClientFactory httpClientFactory, IOptions<AppSettings> settings)
        {
        ...
        public async Task<List<IncomeStatement>> GetIncomeStatements(string symbol)
        {
            ...
            var result = ser.Deserialize<List<IncomeStatement>>(jsonReader);
            return result;
        }

        private string GetUrl(string symbol)
        {
            var result = $"https://financialmodelingprep.com/api/v3/cash-flow-statement/{symbol}?limit=10&apikey={_settings.ApiToken}";
            return result;
        }
    }
}
